...
export abstract class BaseDocumentAgent<T extends z.ZodTypeAny> {
  ...
  async extract(targetImageBase64: string): Promise<z.infer<T>> {
...
Usa la imagen 1 como guía visual para ubicar cada campo en la imagen 2. Extrae ÚNICAMENTE los campos que sean legibles. Si un campo no está presente o no es legible, devuélvelo como null.
...
- Grupo sanguíneo: usa el símbolo (ej: "O+", "A-").`,
        ...
        {
          type: 'image_url',
          image_url: { url: `data:image/png;base64,${targetImageBase64}` },
        },
      ],
    });
